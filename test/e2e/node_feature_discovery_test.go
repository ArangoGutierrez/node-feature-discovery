...
func cleanupCRs(ctx context.Context, cli *nfdclient.Clientset, namespace string) {
	...
	nfgs, err := cli.NfdV1alpha1().NodeFeatureGroups().List(ctx, metav1.ListOptions{})
	Expect(err).NotTo(HaveOccurred())

	if len(nfgs.Items) != 0 {
		By("Deleting NodeFeatureGroup objects from namespace " + namespace)
		for _, nfg := range nfgs.Items {
			err = cli.NfdV1alpha1().NodeFeatureGroups().Delete(ctx, nfg.Name, metav1.DeleteOptions{})
			Expect(err).NotTo(HaveOccurred())
		}
	}
}
...
`)
			...
			`)
					...
					Expect(testutils.CreateNodeFeatureGroupRulesFromFile(ctx, nfdClient, "nodefeaturegroup-1.yaml")).NotTo(HaveOccurred())

					By("Verifying NodeFeatureGroup from NodeFeatureGroupRules #1")
					...
					Eventually(func() bool {
						group, err := nfdClient.NfdV1alpha1().NodeFeatureGroups().Get(ctx, "e2e-test-1", metav1.GetOptions{})
						if err != nil {
							return false
						}
