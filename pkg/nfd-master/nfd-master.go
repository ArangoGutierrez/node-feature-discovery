...
func (m *nfdMaster) nfdAPIUpdateAllNodeFeatureGroups() error {
	if m.nfdController == nil || m.nfdController.featureGroupLister == nil {
		return nil
	}
...
func (m *nfdMaster) nfdAPIUpdateNodeFeatureGroup(nodeFeatureGroupName string) error {
	...
	nodeFeatureGroup, err := nfdClient.NfdV1alpha1().NodeFeatureGroups().Get(context.Background(), nodeFeatureGroupName, metav1.GetOptions{})
	if err != nil {
	...
	nodes := make([]string, 0)
	for _, rule := range nodeFeatureGroup.Spec.FeatureGroupRules {
	...
	if !apiequality.Semantic.DeepEqual(nodeFeatureGroup, nodeFeatureGroupUpdated) {
		...
		nodeFeatureGroupUpdated, err = nfdClient.NfdV1alpha1().NodeFeatureGroups().Update(context.Background(), nodeFeatureGroupUpdated, metav1.UpdateOptions{})
		if err != nil {
